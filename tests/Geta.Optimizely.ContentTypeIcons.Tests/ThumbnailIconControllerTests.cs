...
namespace Geta.Optimizely.ContentTypeIcons.Tests
{
    public class ContentTypeIconControllerTests : IClassFixture<ContentTypeIconControllerFixture>
    {
        ...
        public void Index_FromDisk(int icon, string customFont)
        {
            ...
            // Act
            var result = _fixture.Controller.Index(_fixture.Settings) as FileContentResult;

            // Assert
            Assert.NotNull(result);
            Assert.Equal("image/png", result.ContentType);
            using var image = SKBitmap.Decode(result.FileContents);
        ...
        private static IEnumerable<SKColor> GetUniqueImageColors(SKBitmap image)
        {
